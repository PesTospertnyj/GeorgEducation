/**********************************************
1. Установка Oracle XE 11gR2 , SQL Developer, при установке необходимо придумать и запомнить пароль для SYS
...
2. Настройка workspace  APEX ,
Для настройки нужно зайти по адресу: http://localhost:8080/apex/

Очень наглядно про настройку APEX и developer показано тут
...
select * from emp
---- Эти 3 запроса вернут один и тот же результат--------------
select count(1) from emp
select count(*) from emp
select count(empno) from emp 
----
select count(1) from emp where comm>100 
select count(1) from emp where comm<0 -- count никогда не вернет null, даже если ни одна строка не удовлетворяет условию. Результат запроса будет 0 
...
select deptno, avg(sal), min(sal), max(sal), sum(sal) from emp -- так нельзя, будет ошибка ORA-00937
...
select ename, 
 ...
 ----- результат будет тот же самый, что и у запроса выше -
select dc.cust_first_name, 
        cust_last_name, 
        order_id, 
        order_total
 ...
 select * from demo_customers where customer_id not in (select * from demo_orders) --ошибка, так нельзя (слишком большое кол-во колонок в подзапросе. Должна быть 1 колонка)
 select * from demo_customers where customer_id not in (select order_timestamp from demo_orders) --так тоже нельзя (разные типы у customer_id и order_timestamp)
 ------------
 select * from demo_orders 
